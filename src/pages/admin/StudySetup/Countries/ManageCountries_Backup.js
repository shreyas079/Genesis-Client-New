...
const ManageCountries = ({
  ...
  const warning = () =>
    toast.warn("Select Atleast One Country", {
      toastId: "form-creation",
    });
  ...
  const fetchStudyCountries = async () => {
    try {
      const res = await getStudyCountries();

      if (res.status) {
        console.log("fetchStudyCountries RESSS ... ", res);
      }
    } catch (err) {
  ...
  const onSubmit = async () => {
    try {
      ...
      //     const { shortName, name, notes, regionId, isoId } = filterData[i];
      //     const tempData = {
      //       shortName,
      //       name,
      //       notes,
      ...
      //   const res = await addStudyCountryFromSystem(temp);

      //   console.log("addStudyCountryFromSystem RESSS ... ", res.data);
